...
Climate systems,
such as the monsoons or El Nino/La Nina,
...
Fortunately,
we can bypass this last issue by reexamining the problem.
...
_(a) The two starting patterns from which observations are generated_
...
We leave a detailed discussion of the causes of this mixing to
...
but ratios of their occurrence is different,
...
(issue *O2*),
it is not immediately apparent in this image for two reasons:

- In contrast to the more balanced synthetic example,
...
shows a strong east west contrast with both positive
(red)
and negative
(blue)
values.
...
Inverting the vector's values doesn't solve the issue as it would create other negative zones.
Just by noting this,
...
We know from our synthetic example,
...
In other words the two climates do not cooccur.
...
We then find which bisecting line which maximizes the inter class variance;
...
The error estimation and weighting are explained in greater detail in
link:./paper1-erroranalysis.html[SUPP Error Analysis].
...
Returning to our original thesis.
More importantly,
these patterns allow us to objectively estimate the presence of climate in past and future observation.

Reflecting on the prior art of dipole climate indices,
we first observe that at times it is possible to construct a properly tuned region where EOF1
...
An example of this can be observed in precipitation records.
Wind reversals,
in combination with orographic effects,
will often create mirrored anomalies across mountain ranges
...
We don't need to characterize the climate or construct heuristics,
...
this greatly widens the net of where climate systems are observable.
...
The final patterns are visibly suboptimal due to breakdowns in the assumptions.
...
since the SVD describes observations as sums of static patterns.
A dynamic process,
...
This issue can often be mitigated because at a small enough scales
...
The second issue is that of cooccurrant tertiary climate systems.
...
Here we know a priori that there is both a short-period MJO system as well as the interannual ENSO signal.
